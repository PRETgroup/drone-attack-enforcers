...
class Mission:
	...
	## Determine if at the target position 
	def atPosition(self):
		nextwaypoint=self.vehicle.commands.next
		self._notify("Distance to waypoint (" + str(nextwaypoint) + "): " + str(self.distance_to_current_waypoint()))

		if self.distance_to_current_waypoint() <= 1:
			self._notify("At location")
			return True
		return False
...
class Manager:
	def __init__(self, pipe_c, log=False):
		...
		self.log = log
		if log:
			self.logger = DroneStatusLogger("A2B2A")
		...
		while not req_exit:
			...
			if self.l == "L_DISCONNECT":
				...
				if request[1] == "CONFIG_LOC":
					try:
						self.alt = int(request[2])
						self.d_n = int(request[3])
						self.d_e = int(request[4])
						mission.configure(self.alt, self.d_n, self.d_e)
				...
				if mission.atAltitude():
					...
					time.sleep(1) # TODO: replace with a check that the begin_travel has successfully loaded the new wp
			...
			elif self.l == "L_LANDING_B":
				if not self.log: time.sleep(1) # To slow things down
				if mission.hasLanded():
					self.pipe.send([asctime(localtime()), "LANDED"])
					self.l = "L_WAIT_B"

			elif self.l == "L_WAIT_B":
				if not self.log: time.sleep(1) # To slow things down
				if request[1] == "RETURN_A":
					self._notify("Send")
					mission.configure(self.alt, -self.d_n, -self.d_e)
					self._notify("Done configure")
				...
				if mission.atAltitude():
					mission.begin_Travel()
					self._notify("Begin return trip to A")
					self.l = "L_RETURN_A"
					time.sleep(1) # TODO: replace with a check that the begin_travel has successfully loaded the new wp

			elif self.l == "L_RETURN_A":
				if not self.log: time.sleep(1) # To slow things down
				if mission.atPosition():
					mission.begin_Landing()
					self.l = "L_LANDING"
